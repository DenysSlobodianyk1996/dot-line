...
- **Game page flow** (`src/pages/dot-line-game/`): `Page.vue` owns a single `reactive(new DotLineGame())` and picks the child via `<component :is>` from a computed config (`component`, `props`, `listeners`):
  ...
  - Both screens end with `rules/RulesToggle.vue`: a Show/Hide rules button wrapped around the shared `rules/GameRules.vue`, open by default on setup (`default-open`) and closed during a game. The rules text in `GameRules.vue` must match the spec (RL-5).
...
- **i18n** (`src/i18n/`): vue-i18n with the Composition API (`legacy: false`). `locales/en.ts` defines the message keys, and `locales/uk.ts` is typed as `typeof en`, so both must have the same keys. Ukrainian is the default; `setLocale()` saves `LOCALE` in localStorage and updates `<html lang>` and `document.title`. All UI text goes through `const { t } = useI18n()`; never hard-code visible strings, and add every new string to both locale files. vee-validate messages come from `configure({ generateMessage })` in `src/i18n/index.ts`, using each `<Field>`'s translated `label`. The UA/EN toggle is `src/shared/components/LanguageToggle.vue` in the `App.vue` header.
